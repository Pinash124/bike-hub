    ...
    const fetchBikes = async () => {
      ...
      try {
        ...
        const mappedBikes = activeListings.map((listing: Listing) => ({
          ...
          title: listing.title,
          price: listing.price,
          discipline: listing.bikeType || "N/A",
        ...
        setAllBikes(mappedBikes);
      } catch (error) {
        console.error("Failed to fetch bikes for FilterSection:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const filteredBikes = useMemo(() => {
    return allBikes.filter((bike) => {
      const matchDiscipline =
        filters.discipline === "All" || bike.discipline === filters.discipline;
      const matchBrand =
        filters.brand === "All Brands" || bike.brand === filters.brand;
      const matchCondition =
        filters.condition === "All" || bike.condition === filters.condition;
          ...
          <div className="mb-16 p-10 bg-slate-50/40 backdrop-blur-sm border border-slate-100 rounded-[3rem] animate-in fade-in slide-in-from-top-6 duration-700">
              ...
              {[
                {
                  key: "discipline",
                  label: "Dòng xe",
                  options: ["All", "MTB", "Road", "Gravel", "E-Bike"],
                },
                {
                  key: "brand",
                  label: "Thương hiệu",
                ...
                },
                {
                  key: "condition",
                  label: "Tình trạng",
                  options: ["All", "New", "Refurbished", "Used"],
                },
              ].map((group) => (
                <div key={group.key} className="space-y-3">
                  ...
                  <div className="relative group">
                    <select
                      ...
                      onChange={(e) =>
                        setFilters({ ...filters, [group.key]: e.target.value })
                      }
                    ...
                    <ChevronDown
                      size={14}
                      className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none group-focus-within:text-green-500 transition-colors"
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="mt-10 pt-8 border-t border-slate-100/60 flex items-center justify-between">
              <p className="text-[9px] text-slate-300 font-medium italic">
                * Kết quả tự động cập nhật khi bạn thay đổi bộ lọc.
              </p>
              <button
                onClick={resetFilters}
              ...
              >
                <RotateCcw
                  size={14}
                  className="group-hover:rotate-[-120deg] transition-transform"
                />
                Làm mới tất cả
              </button>
            </div>
          ...
          {isLoading ? (
            Array(4)
              ...
              .map((_, i) => (
                <div
                  key={i}
                  className="aspect-square bg-slate-50 animate-pulse rounded-2xl"
                />
              ))
            ...
            <div className="col-span-full py-24 flex flex-col items-center justify-center animate-in fade-in duration-700">
              ...
              <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">
                Rất tiếc, không tìm thấy xe
              </h3>
              <p className="text-[11px] text-slate-400 mt-2 font-bold uppercase tracking-widest">
                Hãy thử nới rộng khoảng giá hoặc chọn hãng khác bạn nhé!
              </p>
              <button
                onClick={resetFilters}
