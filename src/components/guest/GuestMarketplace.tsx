...
import { Search, Filter, Grid3x3, List, ChevronDown, Sparkles, Bike } from 'lucide-react';
...
export const GuestMarketplace: React.FC = () => {
  ...
  const filteredBikes = bikes.filter(bike => {
    const matchesBrand = selectedBrand === 'Tất cả' || bike.brand?.name === selectedBrand;
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span>
                    {sortBy === 'newest' ? 'Mới nhất' : sortBy === 'price-low' ? 'Giá thấp → cao' : 'Giá cao → thấp'}
                  </span>
                  ...
                  <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl border border-slate-200 shadow-lg z-50 overflow-hidden">
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left text-sm font-medium hover:bg-green-50 transition-colors ${
                        sortBy === 'newest' ? 'bg-green-50 text-green-700' : 'text-slate-700'
                      }`}
                    >
                      Mới nhất
                    </button>
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left text-sm font-medium hover:bg-green-50 transition-colors ${
                        sortBy === 'price-low' ? 'bg-green-50 text-green-700' : 'text-slate-700'
                      }`}
                    >
                      Giá thấp → cao
                    </button>
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left text-sm font-medium hover:bg-green-50 transition-colors ${
                        sortBy === 'price-high' ? 'bg-green-50 text-green-700' : 'text-slate-700'
                      }`}
                    >
                      Giá cao → thấp
                    </button>
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-bold text-slate-900">
                  {selectedBrand === 'Tất cả' ? 'Tất cả sản phẩm' : `Xe ${selectedBrand}`}
                </h3>
              </div>
            ...
            {searchQuery && (
              <div className="flex items-center gap-2 text-sm text-slate-600">
                ...
                <span className="font-semibold text-green-600">"{searchQuery}"</span>
              </div>
            )}
          </div>
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="mt-4 text-sm font-medium text-slate-600">Đang tải danh sách xe...</p>
            </div>
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-bold text-slate-900 mb-2">Không tìm thấy xe phù hợp</h3>
