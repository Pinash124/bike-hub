...
export default function CreateListingPage() {
    ...
    const priceFormatted = priceNum > 0 ? priceNum.toLocaleString('vi-VN') + ' ₫' : null;
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-6">
                <div className="bg-white rounded-3xl shadow-2xl p-12 text-center max-w-md w-full">
                    ...
                    <h2 className="text-2xl font-black text-slate-900 mb-2">Đăng tin thành công!</h2>
                    <p className="text-slate-500 text-sm mb-6">Xe của bạn đang chờ kiểm duyệt. Chúng tôi sẽ xét duyệt trong thời gian sớm nhất.</p>
            ...
            <div className="sticky top-0 z-10 bg-white border-b border-slate-100 shadow-sm">
                <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2.5">
                        <span className="font-black text-slate-800 text-sm uppercase tracking-wide">Đăng xe bán</span>
                    </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            <div className="px-6 py-4 border-b border-slate-50 bg-gradient-to-r from-slate-50 to-white flex items-center gap-2">
                                <h2 className="text-sm font-black text-slate-700 uppercase tracking-wider">Thông tin cơ bản</h2>
                            </div>
                            <div className="p-6 space-y-5">
                                <Field label="Tiêu đề" required hint="Ví dụ: Trek Marlin 5 2022 — đầy đủ tên + năm SX">
                                    <input
                                        required
                                        name="title"
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            <div className="px-6 py-4 border-b border-slate-50 bg-gradient-to-r from-slate-50 to-white flex items-center gap-2">
                                <h2 className="text-sm font-black text-slate-700 uppercase tracking-wider">Chi tiết kỹ thuật</h2>
                            </div>
                            <div className="p-6 space-y-5">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <Field label="Số khung" required hint="Thường khắc dưới gầm khung xe">
                                        <input
                                            required
                                            name="frameNumber"
                                ...
                                <Field label="Mô tả chi tiết" required hint="Tình trạng xe, phụ kiện đi kèm, lịch sử bảo dưỡng...">
                                    <textarea
                                        required
                                        name="description"
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                            <div className="px-6 py-4 border-b border-slate-50 bg-gradient-to-r from-slate-50 to-white flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <h2 className="text-sm font-black text-slate-700 uppercase tracking-wider">Hình ảnh</h2>
                                </div>
                                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${images.length >= 3 ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                                    {images.length >= 3 ? '✓ Đủ ảnh' : `Còn thiếu ${3 - images.length} ảnh`}
                                </span>
                            </div>
                            <div className="p-6">
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 mb-4">
                                    {imagePreviews.map((src, i) => (
                                        <div key={i} className="relative group aspect-square rounded-xl overflow-hidden border-2 border-slate-100">
                                    ...
                                    <label className="aspect-square rounded-xl border-2 border-dashed border-slate-200 hover:border-green-400 hover:bg-green-50 flex flex-col items-center justify-center cursor-pointer transition-all group">
                                        <input
                                            ...
                                            className="hidden"
                                        />
                                        <span className="text-[10px] font-bold text-slate-300 group-hover:text-green-500 uppercase tracking-wide transition-colors">Thêm ảnh</span>
                                    </label>
                                </div>
                                <p className="text-[11px] text-slate-400 flex items-center gap-1">
                                    Ảnh đầu tiên sẽ là ảnh bìa hiển thị cho người mua. Tối thiểu 3 ảnh.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden sticky top-24">
                            <div className="px-6 py-4 border-b border-slate-50 bg-gradient-to-r from-green-50 to-white flex items-center gap-2">
                                <h2 className="text-sm font-black text-slate-700 uppercase tracking-wider">Giá bán</h2>
                            </div>
                            <div className="p-6 space-y-4">
                                ...
                                <div className="border-t pt-4 space-y-2.5">
                                    {[
                                        ...
                                        { ok: !!formData.usageDuration, label: 'Thời gian sử dụng' },
                                        { ok: formData.description.length > 10, label: 'Mô tả' },
                                        { ok: Number(formData.price) > 0, label: 'Giá bán' },
                                        { ok: images.length >= 3, label: `Ảnh (${images.length}/3)` },
                                    ].map(item => (
                                        <div key={item.label} className="flex items-center gap-2 text-xs">
                                            <div className={`w-4 h-4 rounded-full flex items-center justify-center ${item.ok ? 'bg-green-100' : 'bg-slate-100'}`}>
                                                {item.ok
                                                    ? <span className="text-green-600 font-bold">✓</span>
                                                    : <div className="w-1.5 h-1.5 rounded-full bg-slate-300" />
                                                }
                                            </div>
                                            <span className={item.ok ? 'text-slate-700 font-semibold' : 'text-slate-400'}>{item.label}</span>
                                        </div>
                                    ))}
                                </div>
