...
export default function ChoosePlanPage() {
  ...
  const fetchData = async (id: string) => {
    ...
    try {
      ...
      if (!listing) {
        try {
          const listingData = await listingService.getSellerListingById(id);
      ...
      try {
        ...
        const activePlans = Array.isArray(planData)
          ? planData.filter((p) => p.isActive !== false)
          : [];
        ...
        throw new Error(
          "Lỗi tải gói dịch vụ và đăng ký: " +
  ...
  const handleResumePayment = async () => {
    ...
    try {
      localStorage.setItem(
        ...
        String(currentSubscription.id),
      );
      if (listingId)
        localStorage.setItem("pendingListingId", String(listingId));
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-xs text-slate-500 font-semibold">
                <span className="inline-flex items-center gap-1 bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full ring-1 ring-slate-200">
                  <Tag size={12} /> {data.brand?.name || "Thương hiệu"}
                </span>
                {data.bikeType && (
                  <span className="inline-flex items-center gap-1 bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full ring-1 ring-slate-200">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
              <div className="bg-gradient-to-r from-indigo-600 to-indigo-500 p-6 text-white">
                ...
                <h3 className="text-xl font-black">
                  Gói đăng ký đã được ghi nhận
                </h3>
                <p className="text-indigo-100 text-sm">
                  Bạn đã sẵn sàng thanh toán để kích hoạt tin đăng.
                </p>
              </div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b border-slate-100">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-base font-bold text-slate-900">
                      {selectedPlanDetails.durationDays} ngày
                    </div>
                  </div>
                </div>
                ...
                <p className="mt-6 text-center text-xs text-slate-500">
                  * Tin đăng sẽ hiển thị chính thức sau khi Admin xác nhận thanh
                  toán.
                </p>
              </div>
            </div>
            ...
            {plans.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded-2xl p-10 text-center max-w-md mx-auto">
                ...
                <p className="text-slate-600 mb-5 text-sm">
                  Hệ thống đang cập nhật các gói đăng tin mới. Vui lòng quay lại
                  sau.
                </p>
                <button
                  onClick={() => navigate("/seller/dashboard")}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mb-6 text-center">
                          ...
                          <p className="text-slate-600 text-sm min-h-[40px]">
                            {plan.description}
                          </p>
                        </div>
                        ...
                        <ul className="space-y-3 mb-6 flex-1">
                          <li className="flex items-start gap-2">
                            <div
                              className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center ${isRecommended ? "bg-indigo-100 text-indigo-600" : "bg-emerald-100 text-emerald-600"}`}
                            >
                              <Check size={12} strokeWidth={4} />
                            </div>
                            <span className="text-sm text-slate-700">
                              Đăng bán công khai {plan.durationDays} ngày
                            </span>
                          </li>
                          <li className="flex items-start gap-2">
                            <div
